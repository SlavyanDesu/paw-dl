...
- `src/index.ts` processes posts sequentially; `src/downloader/queue.ts` limits concurrent file downloads to 3 (`DEFAULT_CONCURRENCY` lives there). Creator listings paginate in batches of 50 and fetch each post's detail separately. Favorites mode reuses the same per-post path: full post objects download directly, each favorited creator is walked like a creator URL.
- `src/cli.ts`: output defaults to cwd (`-o <folder>` takes a required value). `--post <n>` limits posts per creator, or favorited posts in posts scope; omitting it fetches all posts. `--favorites <posts|creators>` takes no URL and needs `--session`/`PAWCHIVE_SESSION`. `--flat` works with creator URLs and both favorites scopes (root manifest identities `["favorites","posts"]` / `["favorites","creators"]`). Neither option works with single-post URLs. `--include-files zip,psd` (or `all`) adds to the default image/video filter; deferred attachments are always skipped.
- `src/api/client.ts` uses `https://pawchive.pw/api/v1`; files use `https://file.pawchive.pw/data/...`. Favorites list from `GET /account/favorites` (creators) and `?type=post` (full post objects, unpaginated); session goes as a `Cookie` header, 401/403 with session means expired. Retryable statuses (408/429/500/502/503/504) and `Retry-After` parsing live in `src/utils/http.ts`; the shared backoff loop lives in `src/utils/retry.ts` and is used by both the API client and the file downloader. `FILE_ORIGIN` lives in `src/utils/attachment-url.ts`, which also owns URL validation and query-free source identity.
