...
- No `commander`/`cli-progress`: CLI parsing is `node:util parseArgs` in `src/cli.ts`, progress is a no-op `ProgressTracker` in `src/downloader/progress.ts`. Don't reintroduce them.
