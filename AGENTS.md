...
- `src/cli.ts`: output defaults to cwd (`-o <folder>` takes a required value). `--post <n>` and `--flat` work on creator URLs only; omitting `--post` fetches all posts. `--favorites <posts|creators|all>` takes no URL and needs `--session`/`PAWCHIVE_SESSION`; `--flat` is rejected with it. `--include-files zip,psd` (or `all`) adds to the default image/video filter; deferred attachments are always skipped.
